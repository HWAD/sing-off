  ...
  public void startRecorder(int numChannels, int sampleRate, Integer bitRate, int androidEncoder, String path, final Result result) {
    ...
    */
    if (this.model.getMediaRecorder() == null) {
      ...
      this.model.getMediaRecorder().setOutputFormat(MediaRecorder.OutputFormat.AMR_WB);//OutputFormat.MPEG_4);
      this.model.getMediaRecorder().setAudioEncoder(MediaRecorder.AudioEncoder.AMR_WB);//androidEncoder);
      this.model.getMediaRecorder().setAudioChannels(numChannels);
      this.model.getMediaRecorder().setAudioSamplingRate(sampleRate);

      //test this.model.getMediaRecorder().setOutputFile(path);
    ...
    try {
      ...
      this.model.getMediaRecorder().setOutputFile(parcelWrite.getFileDescriptor());
